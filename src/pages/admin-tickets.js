...
function Event() {
  ...
  const fetch = () => {
      ...
      supabase.from("tickets").select(`*, event(*)`).then(events => {
        setCompState({
          ...compState, tickets:events.body, loading:false
        })
      }).catch(error => {
        setCompState({
  ...
  let events = () => {
    if (compState.tickets) {
       return compState.tickets.map(evts => {
         console.log(evts)
        return (
          <>
            <tr style={{cursor:"no-drop"}}>
              <th>{evts.id}</th>
              <th>{evts.event.event_title}</th>
              <th>{evts.event.seller_name}</th>
              <th>{evts.meta.amount}</th>
              <th>{evts.meta.category}</th> 
              <th>{evts.event.meta.location}</th>
            </tr>
            </>
        )
      })
